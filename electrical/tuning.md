...
- **時間範圍**：2026年8月4日—2026年8月18日
- **記錄依據**：本次對話中已經進行的硬件、軟件、STM32、GitHub、傳感器、循跡、PID、避障、OLED、藍牙及樹莓派相關工作。
- **說明**：對話中明確出現「已測試/出現結果」的內容按實際結果記錄；只有代碼編寫、參數設計但沒有明確實車測試結果的部分，標記為「已編寫，待實測」，避免把計劃誤寫成測試結果。除此之外，由於後期重新把所有用HAL庫格式寫的文件又用標註庫編了一遍，前面提到編寫過程時，對於同名或類似文件，就不引用原來文件的鏈接。
...
**編寫**：[USART.c(electrical/USART.c);
          [USART.h](electrical/USART.h)
...
開始編寫：[LED.c](electrical/LED.c);
         [LED.h](electrical/LED.h)
...
建立：[MPU6050.c](electrical/HARDWARE/MPU6050/mpu6050.c);
     [MPU6050.h](electrical/HARDWARE/MPU6050/mpu6050.h);
     [MPU6050_angle.c](electrical/HARDWARE/MPU6050/mpu6050_angle.c);
     [MPU6050_angle.h](electrical/HARDWARE/MPU6050/mpu6050_angle.h)
...
建立：[HC_SR04.c](electrical/HCSR04/HCSR04.c);
     [HC_SR04.c](electrical/HCSR04/HCSR04.h);
     [Avoidance](electrical/HARDWARE/Avoidance/avoidance.c);
     [Avoidance](electrical/HARDWARE/Avoidance/avoidance.h)
...
建立：[Trace](electrical/Trace/Trace.c）；
      [Trace](electrical/Trace/Trace.h)
...
### 修改內容
建立：[pid.c](electrical/HARDWARE/PID/pid.c);
     [pid.h}(electrical/HARDWARE/PID/pid.h)
...
### 工作內容
增加 OLED：[OLED.c](electrical/OLED/OLED.c);
          [OLED.h](electrical/OLED/OLED.h)；
          [OLEDIIC.c](electrical/OLED/OLEDIIC.c)；
          [OLEDIIC.h](electrical/OLED/OLEDIIC.h)；
          [word.h](electrical/OLED/word.h)
...
## 2026年8月16日
工程命令與運行行為檢查，以及小車控制app的編寫

### 工作內容
繼續確認程序運行命令、燒錄行為和工程執行狀態，避免代碼正確，但運行配置錯誤，最終導致實際小車不工作的問題。
編寫了[STM32智能车遥控](electrical/STM32智能车遥控.apk)
...
### 工作內容
系統整理了STM32CubeMX、Keil、HAL、GPIO、TIM、USART、I2C等基礎配置，形成與實際 PCB/原理圖對應的配置方案。
懷疑是小車設備不能支持標准庫，重新修改了一個標准庫的文件版本。
...
### 工作內容
利用app實現了開燈、關燈、前進、後退、左轉、右轉、停止、緊急停止、避障、循跡。
此後修改了循跡部分的代碼，修改了轉彎時電機的轉速（負方向的小一點，正方向的大一點），增加了只有X2亮和只有X4亮的情況。
...
之前已經出現過的「轉彎後左右抖動」「避障啟動後容易原地打轉」問題等，已經緩解，並改善了部分功能。
